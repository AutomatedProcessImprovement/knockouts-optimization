...
class TaskEvent:
    ...
    def complete_event(self, ended_at, waiting_time):
    ...
    def processing_time(self):
        return self.idle_processing_time() - self.waiting_time

    def idle_cycle_time(self):
        return (self.completed_at - self.enabled_at).total_seconds()

    def cycle_time(self):
        return self.idle_cycle_time() - self.waiting_time
...
class Trace:
    def __init__(
        self,
        p_case,
        started_at=datetime.datetime(9999, 12, 31, 23, 59, 59, 999999, pytz.utc),
    ):
        self.p_case = p_case
        self.started_at = started_at
    ...
    def start_event(
        self, task_id, task_name, started_at, started_by, enabled_at, enabled_by
    ):
    ...
    def complete_event(self, event_index, completed_at, waiting_time=0):
        self.event_list[event_index].complete_event(completed_at, waiting_time)
        self.completed_at = max(
            self.completed_at, self.event_list[event_index].completed_at
        )
...
class BPMNGraph:
    ...
    def postprocess_unfired_tasks(
        ...
        task_sequence = [
            task_name for task_name in task_sequence if task_name in self.from_name
        ]
    ...
    def compute_branching_probability(self, flow_arcs_frequency):
        ...
        for e_id in self.element_info:
            if (
                self.element_info[e_id].type == BPMNNodeType.EXCLUSIVE_GATEWAY
    ...
    def _recalculate_arcs_probabilities(
        ...
        if (
            ...
            average_probability_to_balance = (
                1 - valid_probabilities - number_of_invalid_arcs * min_probability
            ) / number_of_valid_arcs
            for flow_id in flow_arcs_probability:
                if flow_arcs_probability[flow_id] <= valid_probability_threshold:
                    ...
                    probability = (
                        flow_arcs_probability[flow_id] + average_probability_to_balance
                    )
                flow_arcs_probability[flow_id] = probability
...
class RCalendar:
    ...
    def add_calendar_item(self, from_day, to_day, begin_time, end_time):
        if from_day.upper() in str_week_days and to_day.upper() in str_week_days:
            try:
                t_interval = Interval(
                    self._parse_datetime(begin_time, False),
                    self._parse_datetime(end_time, False),
                )
    ...
    def find_waiting_time(self, requested_date, duration):
        real_duration = 0
        pending_duration = duration
        if duration > self.total_weekly_work:
            real_duration += self._to_seconds(
                int(duration / self.total_weekly_work), "WEEKS"
            )
